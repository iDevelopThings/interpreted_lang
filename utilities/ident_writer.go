...
// An IndentWriter is an io.Writer.  All indenters in an uninterruped chain share
// the same sol value.
type IndentWriter struct {
	...
	p       *IndentWriter // the IndentWriter we wrapped
}

// NewWriter is the name used in github.com/openconfig/goyang/pkg/indent.
var NewWriter = NewIndentWriter
...
func NewIndentWriter(w io.Writer, prefix string) io.Writer {
	...
	// If we are indenting an IndentWriter then we can just combine the
	// indents.
	if in, ok := w.(*IndentWriter); ok {
		return &IndentWriter{
			w:      in.w,
...
//	1> abc123
//	1> 2> 456def
func (in *IndentWriter) Write(buf []byte) (int, error) {
	if len(buf) == 0 {
		return 0, nil
	}
	sol := *in.sol
...
// Unwrap unwraps and IndentWriter and returns the underlying io.Writer.  It will
// unwrap up to n times or until an io.Writer that is not an IndentWriter is
