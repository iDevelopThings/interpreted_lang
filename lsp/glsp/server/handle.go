...
func (self *Server) handle(context contextpkg.Context, connection *jsonrpc2.Conn, request *jsonrpc2.Request) (any, error) {
	glspContext := glsp.Context{
		...
		Notify: func(method string, params any) {
			self.Log.Infof("notification: %s, data: %s", method, params)
	...
	self.Log.Infof("Received request: %s, data: %s", request.Method, request.Params)
	...
	default:
		...
		if err != nil {
			self.Log.Errorf("Error handling request: %s", err.Error())
		}
